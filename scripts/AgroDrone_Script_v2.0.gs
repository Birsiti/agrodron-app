// ══════════════════════════════════════════════════════════
// AgroDrone.BY — Google Apps Script v2.0
// ЗАО «АТК» | май 2026
// ══════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════
// СТРУКТУРА ЛИСТОВ (реальная таблица)
// ══════════════════════════════════════════════════════════
//
...
//
// ПОЛЯ КЛИЕНТОВ: A=ID B=ID клиента C=Название поля
//   D=Ссылка Maps E=Культура F=Препарат G=Норма л/га
//   H=Площадь га I=Статус J=Фото URL K=Дата закрытия
//   L=ID обработки M=Примечания
//
...
//
// ЗАЯВКИ: A=ID заявки B=Дата C=Организация D=УНП E=Телефон
...
// ══════════════════════════════════════════════════════════
// ТОЧКА ВХОДА — GET (проверка работоспособности)
// ══════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════
// ПОЛЯ КЛИЕНТОВ
// A=ID B=ID клиента C=Название D=Ссылка Maps E=Культура
// F=Препарат G=Норма л/га H=Площадь га I=Статус
// J=Фото URL K=Дата закрытия L=ID обработки M=Примечания
// ══════════════════════════════════════════════════════════
...
function addField(data) {
  ...
  sheet.appendRow([
    ...
    data.clientId   || '',
    data.name       || '',
    data.mapsUrl    || data.coords || '',
    data.crop       || '',
    data.prep       || '',
    normalizeNumber(data.norm) || '',
    parseFloat(normalizeNumber(data.haPlan)) || 0,
    'новое',   // I=Статус
    '',        // J=Фото URL
    '',        // K=Дата закрытия
    '',        // L=ID обработки
  ...
  // Уведомление менеджеру в группу
...
function closeField(data) {
  // data: { id, haToday, isFullyDone, operatorIds, bort, agroTelegramId }
  const sheet = getSheet('ПОЛЯ КЛИЕНТОВ');
  const found = findRowById(sheet, data.id);
  if (!found) return { error: 'Поле не найдено: ' + data.id };

  const { rowNum, rowData } = found;
  const haPlan    = parseFloat(rowData[7]) || 0;
  ...
  sheet.getRange(rowNum, 9).setValue(status); // I=Статус
  if (isDone) sheet.getRange(rowNum, 11).setValue(now); // K=Дата закрытия

  // Создать запись в ОБРАБОТКИ
  ...
  // Записать ID обработки в поле
  sheet.getRange(rowNum, 12).setValue(obrabotkaId); // L=ID обработки

  const fieldName = rowData[2];
  const crop      = rowData[4];
...
// ══════════════════════════════════════════════════════════
// КЛИЕНТЫ
// A=ID B=Хозяйство C=Агроном ФИО D=Telegram ID E=Телефон
// F=Email G=Статус H=Дата активации I=Менеджер J=Примечания
// ══════════════════════════════════════════════════════════
...
function getClient(data) {
  ...
  return {
    ...
    telegramId: String(r[3]), phone: r[4], status: r[6]
  };
}

// ══════════════════════════════════════════════════════════
// КОМАНДИРОВКИ
// A=ID B=Дата откр. C=Дата закр. D=Район E=Состав
// F=Техника G=Аванс H=Потрачено I=Остаток J=Га итого
// K=Статус L=Закрытие M=Примечание
// ══════════════════════════════════════════════════════════
...
function getTrips(data) {
  ...
  const trips = rows.map(r => ({
    id:        String(r[0]),
    dateStart: r[1], dateEnd: r[2], direction: r[3],
    crew:      r[4], equip:   r[5],
    advance:   r[6], spent:   r[7], balance:  r[8],
...
function closeTrip(data) {
  ...
  sheet.getRange(rowNum, 3).setValue(now);           // C=Дата закр.
  sheet.getRange(rowNum, 8).setValue(spent);          // H=Потрачено
  sheet.getRange(rowNum, 9).setValue(balance);        // I=Остаток
  sheet.getRange(rowNum, 10).setValue(data.ha || 0); // J=Га итого
  sheet.getRange(rowNum, 11).setValue('закрыта');     // K=Статус
  sheet.getRange(rowNum, 12).setValue(data.closeOption || ''); // L
  if (data.note) sheet.getRange(rowNum, 13).setValue(data.note); // M
...
// ══════════════════════════════════════════════════════════
// ДНИ
// A=ID B=Командировка ID C=День№ D=Дата E=Га план
// F=Га факт G=Полей закрыто H=Полей перенесено
// I=Расходы за день J=Статус K=Примечание
// ══════════════════════════════════════════════════════════
  ...
  if (data.fieldIds && data.fieldIds.length > 0) {
    ...
    data.fieldIds.forEach(fid => {
      for (let i = 1; i < pkData.length; i++) {
        if (String(pkData[i][0]) === String(fid) && pkData[i][8] !== 'готово') {
          pkSheet.getRange(i + 1, 9).setValue('в работе');
        }
      }
    });
  }
...
function closeDay(data) {
  // data: { dayId, tripId, dayNum, date, haDone, fieldsDone,
  //         fieldsCarried, spentToday, totalSpent, totalHa, note }
  const sheet = getSheet('ДНИ');
  const found = findRowById(sheet, data.dayId);

  if (found) {
    sheet.getRange(found.rowNum, 6).setValue(data.haDone       || 0); // F
    sheet.getRange(found.rowNum, 7).setValue(data.fieldsDone   || 0); // G
    sheet.getRange(found.rowNum, 8).setValue(data.fieldsCarried|| 0); // H
    sheet.getRange(found.rowNum, 9).setValue(data.spentToday   || 0); // I
    sheet.getRange(found.rowNum, 10).setValue('закрыт');               // J
    if (data.note) sheet.getRange(found.rowNum, 11).setValue(data.note); // K
  }

  // Обновить командировку (потрачено + га)
  if (data.tripId) {
    ...
    if (tripFound) {
      ...
      tripSheet.getRange(tripFound.rowNum, 8).setValue(data.totalSpent || 0); // H
      tripSheet.getRange(tripFound.rowNum, 9).setValue(adv - (data.totalSpent || 0)); // I
      tripSheet.getRange(tripFound.rowNum, 10).setValue(data.totalHa  || 0); // J
    }
  }
...
// ══════════════════════════════════════════════════════════
// РАСХОДЫ
// A=ID B=Командировка ID C=День ID D=Дата E=Время
// F=Категория G=Сумма H=Комментарий
// ══════════════════════════════════════════════════════════
...
function addExpense(data) {
  ...
  sheet.appendRow([
    ...
    formatTime(now),
    data.label   || data.cat || '',
    parseFloat(normalizeNumber(data.amount)) || 0,
    data.comment || ''
  ]);
  ...
  if (data.tripId && data.totalSpent !== undefined) {
    ...
    if (found) {
      ...
      tripSheet.getRange(found.rowNum, 8).setValue(sp);       // H=Потрачено
      tripSheet.getRange(found.rowNum, 9).setValue(adv - sp); // I=Остаток
    }
  }
...
// ══════════════════════════════════════════════════════════
// ОБРАБОТКИ (журнал выполненных работ)
// A=ID B=Дата C=ID заявки D=НП E=Культура F=Препарат
// G=Норма л/га H=Га факт I=Борт J=Операторы
// K=Га/оператор L=Фото URL M=Статус N=Дата закрытия O=Примечания
// ══════════════════════════════════════════════════════════
...
function addObrabotka(data) {
  ...
  // Га/оператор — делим поровну на всех операторов
  const opsArr = data.operators
    ? data.operators.split(',').map(s => s.trim()).filter(Boolean)
    : [];
  ...
  sheet.appendRow([
    id,
    now,                     // B=Дата
    data.zayavkaId || '',    // C=ID заявки
    data.fieldName || '',    // D=НП
    data.crop      || '',    // E=Культура
    data.prep      || '',    // F=Препарат
    normalizeNumber(data.norm) || '', // G=Норма л/га
    ha,                      // H=Га факт
    data.bort      || '',    // I=Борт
    data.operators || '',    // J=Операторы
    haPerOp,                 // K=Га/оператор
    data.photoUrl  || '',    // L=Фото URL
    'закрыто',               // M=Статус
    now,                     // N=Дата закрытия
    data.note      || ''     // O=Примечания
  ]);

  // Обновить статистику операторов
  if (opsArr.length > 0) {
    updateOperatorStats({ ids: opsArr, haPerOp });
  }
...
function getObrabotki(data) {
  ...
  const list  = rows.map(r => ({
    ...
    crop: r[4], prep: r[5], norm: r[6], haFact: r[7],
    bort: r[8], operators: r[9], haPerOp: r[10],
    photoUrl: r[11], status: r[12], dateClosed: r[13], note: r[14]
  }));
...
// ══════════════════════════════════════════════════════════
// ОПЕРАТОРЫ
// A=ID B=ФИО C=Телефон D=Район E=Серт.БАК F=Часов
// G=Статус H=Дата рег. I=Telegram ID J=Тип
// K=Га за сезон L=Выездов M=Роль N=Дата посл.выезда
// ══════════════════════════════════════════════════════════
  ...
  (data.ids || []).forEach(opId => {
    for (let i = 1; i < rows.length; i++) {
      if (String(rows[i][0]) === String(opId)) {
        ...
        sheet.getRange(i+1, 11).setValue(curHa + (data.haPerOp || 0)); // K=Га за сезон
        sheet.getRange(i+1, 12).setValue(curTrips + 1);                // L=Выездов
        sheet.getRange(i+1, 14).setValue(now);                         // N=Дата посл.выезда
        break;
      }
    }
  });
...
// ══════════════════════════════════════════════════════════
// ЗАЯВКИ
// A=ID заявки B=Дата C=Организация D=УНП E=Телефон
// F=Telegram ID G=Район/НП H=Площадь I=Культура ...
// Q=Статус R=Дата статуса X=Заметки менеджера
// ══════════════════════════════════════════════════════════
...
function getZayavki(data) {
  ...
  const list  = rows.map(r => ({
    ...
    priceTotal: r[21], pricePerHa: r[22], managerNote: r[23],
    distance: r[24]
...
function updateZayavka(data) {
  // data: { id, status, managerNote? }
  const sheet = getSheet('ЗАЯВКИ');
  const found = findRowById(sheet, data.id);
  if (!found) return { error: 'Заявка не найдена: ' + data.id };

  const now = formatDate(new Date());
  if (data.status)      sheet.getRange(found.rowNum, 17).setValue(data.status);     // Q
  if (data.status)      sheet.getRange(found.rowNum, 18).setValue(now);             // R
  if (data.managerNote) sheet.getRange(found.rowNum, 24).setValue(data.managerNote); // X
...
// ══════════════════════════════════════════════════════════
// КАНДИДАТЫ
// A=ID B=Дата C=ФИО D=Возраст E=Район F=Телефон G=Роль
// H=Опыт I=Авто J=Выезды K=Источник L=О себе
// M=Telegram ID N=Статус O=Дата статуса P=Заметки
// ══════════════════════════════════════════════════════════
...
function updateKandidat(data) {
  ...
  const now = formatDate(new Date());
  if (data.status) sheet.getRange(found.rowNum, 14).setValue(data.status); // N
  if (data.status) sheet.getRange(found.rowNum, 15).setValue(now);         // O
  if (data.note)   sheet.getRange(found.rowNum, 16).setValue(data.note);   // P
...
// ══════════════════════════════════════════════════════════
// БРИГАДЫ
// A=ID B=Дата создания C=Старший(TG) D=Операторы(TG)
// E=Техник(TG) F=ID выезда G=Статус H=Дата завершения I=Примечания
// ══════════════════════════════════════════════════════════
