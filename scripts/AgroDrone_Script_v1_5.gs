...
const TELEGRAM_BOT_TOKEN = '8675713209:AAHS6P8TGpO3efMHwDblTzyLwDarHMINiAk'; // @AgroDrone_BY_Bot
const GROUP_CHAT_ID      = '-1003761125572';   // Агрообработки 2026 (боевой)
const OWNER_CHAT_ID      = '7181989671';     // Денис — супервизор, видит всё
const MANAGER_CHAT_ID    = 'УЗНАТЬ_ID';      // Менеджер — заменить после проверки
// Агроном — берётся из профиля (agroTelegramId в запросе)
...
const HEADERS = {
  ...
  ОБРАБОТКИ: [
    'ID','Дата','ID заявки','НП','Культура','Препарат',
    'Норма л/га','Га факт','Борт','Операторы','Га/оператор',
    'Фото URL','Статус','Дата закрытия','Примечания'
  ],
  // Индексы колонок ОБРАБОТКИ (0-based):
  // 0=ID 1=Дата 2=ID заявки 3=НП 4=Культура 5=Препарат
  // 6=Норма л/га 7=Га факт 8=Борт 9=Операторы 10=Га/оператор
  // 11=Фото URL 12=Статус 13=Дата закрытия 14=Примечания
  ДНИ: [
...
function addField(data) {
  ...
  const id    = `F-${dd}${mm}${yy}-${hh}${min}`; // F-120526-1604
  // Нормализуем норму: запятая → точка
  const norm  = String(data.norm || '').replace(',', '.');

  // Структура: ID | Дата | ID заявки | НП | Культура | Препарат |
  //            Норма л/га | Га факт | Борт | Операторы | Га/оператор |
  //            Фото URL | Статус | Дата закрытия | Примечания
  sheet.appendRow([
    id,                      // A: ID
    formatDate(now),         // B: Дата
    data.tripId  || '',      // C: ID заявки (командировка)
    data.name    || '',      // D: НП (название поля)
    data.crop    || '',      // E: Культура
    data.prep    || '',      // F: Препарат
    parseFloat(norm) || '', // G: Норма л/га
    data.haPlan  || 0,      // H: Га факт (план при создании)
    data.equip   || '',      // I: Борт
    data.agroId  || '',      // J: Операторы (агроном при создании)
    0,                       // K: Га/оператор
    data.coords  || '',      // L: Фото URL (координаты пока)
    'новое',                 // M: Статус
    '',                      // N: Дата закрытия
    ''                       // O: Примечания
  ]);
  ...
  const msgNewField =
    ...
    `🏢 ${data.orgName || '—'} · УНП ${data.unp || '—'}\n` +
    `📍 ${data.region || '—'}\n\n` +
    `📋 ${data.name}\n` +
    `🌾 ${data.crop} · ${data.prep}\n` +
    `📐 ${data.haPlan} га · ${norm || '—'} л/га\n` +
    `${data.coords ? '📍 ' + data.coords + '\n' : ''}` +
    `_Требует подтверждения менеджера_`;
  // Новое поле → менеджеру + супервизору
  notify([MANAGER_CHAT_ID, OWNER_CHAT_ID], msgNewField);
...
function updateFieldStatus(data) {
  ...
  // Новая структура: M=13=Статус, C=3=ID заявки
  sheet.getRange(rowNum, 13).setValue(data.status);           // M: Статус
  if (data.tripId) sheet.getRange(rowNum, 3).setValue(data.tripId); // C: ID заявки
  ...
  if (data.status === 'подтверждено') {
    ...
    // → менеджеру + агроному + супервизору
    notify([MANAGER_CHAT_ID, data.agroTelegramId, OWNER_CHAT_ID], msgConfirm);
  }
  ...
  if (data.status === 'отклонено') {
    ...
    // → менеджеру + агроному + супервизору
    notify([MANAGER_CHAT_ID, data.agroTelegramId, OWNER_CHAT_ID], msgReject);
  }
...
function getFields(data) {
  ...
  const fields = rows.slice(1)
    ...
    .map(r => ({
      ...
      crop:        r[4],
      prep:        r[5],
      norm:        String(r[6] || '').replace(',','.'), // нормализуем
      haPlan:      parseFloat(r[7]) || 0,
      equip:       r[8],   // борт
      agroId:      r[9],   // операторы
      haPerOp:     r[10],
      coords:      r[11],  // фото URL / координаты
      status:      String(r[12] || 'новое').toLowerCase(),
      dateClosed:  r[13],
      notes:       r[14]
    }))
    .filter(f => {
...
function closeField(data) {
  ...
  const { rowNum, rowData } = found;
  const haPlan    = rowData[7];   // H: Га факт (план)
  const haFactOld = rowData[7] || 0;
  ...
  sheet.getRange(rowNum, 13).setValue(status);    // M: Статус
  sheet.getRange(rowNum, 8).setValue(haFactNew);  // H: Га факт
  if (status === 'готово') {
    sheet.getRange(rowNum, 14).setValue(formatDate(new Date())); // N: Дата закрытия
  }

  const fieldName = rowData[3];  // D: НП
  const crop      = rowData[4];  // E: Культура

  if (status === 'готово') {
    const msgDone =
      `✅ *Поле закрыто полностью*\n\n` +
      `📋 ${fieldName}\n🌾 ${crop}\n` +
      `📐 Обработано: *${haFactNew} га* из ${haPlan} га`;
    // → группа + агроном + супервизор
    notify([GROUP_CHAT_ID, data.agroTelegramId, OWNER_CHAT_ID], msgDone);
  } else {
    const msgPartial =
      `⏩ *Поле обработано частично*\n\n` +
      `📋 ${fieldName}\n` +
      `📐 Сегодня: ${data.haToday} га · Всего: ${haFactNew}/${haPlan} га\n` +
      `🔜 Остаток: *${remainder.toFixed(1)} га* — перенесено на завтра`;
    // → агроном + супервизор (не в группу)
    notify([data.agroTelegramId, OWNER_CHAT_ID], msgPartial);
  }
...
function addTrip(data) {
  ...
  // → группа + супервизор
  notify([GROUP_CHAT_ID, OWNER_CHAT_ID], msgTrip);
...
function closeTrip(data) {
  ...
  const msgTripClose =
    ...
    `${data.note ? '\n📝 ' + data.note : ''}`;
  // → группа + супервизор
  notify([GROUP_CHAT_ID, OWNER_CHAT_ID], msgTripClose);
...
function openDay(data) {
  ...
  // → группа + агроном + супервизор
  notify([GROUP_CHAT_ID, data.agroTelegramId, OWNER_CHAT_ID], msgDayOpen);
...
function closeDay(data) {
  ...
  const msgDayClose =
    ...
    `${data.note ? '\n📝 ' + data.note : ''}`;
  // → группа + агроном + супервизор
  notify([GROUP_CHAT_ID, data.agroTelegramId, OWNER_CHAT_ID], msgDayClose);
...
function sendTelegram(chatId, text) {
  ...
  try {
    ...
    if (!result.ok) {
      Logger.log(`Telegram error for ${chatId}: ${result.description}`);
    }
  } catch(e) {
    Logger.log(`Telegram exception for ${chatId}: ${e.message}`);
  }
}

// Отправить нескольким получателям сразу
function notify(chatIds, text) {
  chatIds.forEach(id => { if (id) sendTelegram(id, text); });
}
